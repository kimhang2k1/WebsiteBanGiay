...
class CartController extends Controller
{
        ...
        if (session()->has('user')) {
            ...
            if (count($cart) <= 0) {
                ...
                return response()->json([
                    'view' => " " . view('/User/component/Cart/box_cart')->with('gh', $cart[0]),
    ...
    public function getCart(Request $request)
    {
        if (session()->has('user')) {
            ...
            return view('/User/Main/cart')->with('shoppingCart', $shoppingCart);
        }
    }
    ...
    public function deleteCart(Request $request)
    {
        ...
        if (count($cart) <= 1) {
            ...
            return view('/User/component/Cart/form-not-cart');
        } else {
            DB::table('giohang')->where('STT', '=', $request->STT)->where('IDTaiKhoan', '=', Session::get('user')[0]->IDTaiKhoan)->delete();
            return '';
        }
