...
<div class="w-full flex bg-gray-100 space-x-4">
    ...
    <div class="w-1/2 leading-8 bg-white py-4 px-4 text-sm">
        <div class="w-full">
            ...
            <?php
            $sum = DB::table('chitietdonhang')->where('IDDonHang', '=', $billCustomer[0]->IDDonHang)->sum('ThanhTien');
            ?>
            <div class="w-full flex space-x-60 mb-4">
                <div class="mr-2">
                    Tổng Tiền(<span class="px-1">{{ count($bill) }}</span>sản phẩm)
                </div>
                <div>
                    {{ number_format($sum)}}
                </div>
            ...
            <div class="w-full flex space-x-80 text-sm mb-4">
                ...
                <div class="text-base">
                    {{ number_format($sum + 25000)}}
                </div>
            </div>
            @if($billCustomer[0]->IDTrangThai != 1)
            <div class="text-sm">
                Vui lòng thanh toán khi nhận được hàng
            </div>
            @elseif($billCustomer[0]->IDTrangThai == 1)
            <div class="w-full flex space-x-4 my-4 justify-end pr-8 ">
                <button class="bg-12283e text-white px-4 py-1" type="button"
                    onclick="cancelOrder('{{ $billCustomer[0]->IDDonHang}}')">Hủy Đơn
                    Hàng</button>
            </div>
            @endif
        </div>
